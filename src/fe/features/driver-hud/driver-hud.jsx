...
const PHASE_COLORS = {
  to_incident: 'text-red-400 border-red-600 bg-red-900/20',
  to_hospital: 'text-yellow-400 border-yellow-600 bg-yellow-900/20',
...
export default function DriverHud() {
  ...
  const phaseColor = PHASE_COLORS[currentPhase] || 'text-[#8b8b8b] border-[#333] bg-[#1a1a1a]';
    ...
    <div className="bg-slate-900 text-white min-h-screen w-full flex justify-center">
      <div className="w-full max-w-md h-screen relative bg-[#0b0b0b] overflow-hidden flex flex-col shadow-2xl border-x border-slate-800">
        ...
        <div className="absolute top-4 left-4 right-4 z-20">
          <div className="bg-[#1a1a1a]/95 backdrop-blur-md rounded-2xl shadow-[0_10px_30px_rgba(0,0,0,0.6)] border border-[#2a2a2a] overflow-hidden">

            {/* Phase banner */}
            {hasMission && (
              <div className={`px-4 py-2 border-b border-[#2a2a2a] flex items-center gap-2 text-[10px] font-bold tracking-widest ${phaseColor}`}>
            ...
            <div className="flex justify-between items-center px-4 py-3">
              <div className="flex flex-col items-center">
                <p className="text-[#8b8b8b] text-[9px] font-bold tracking-widest uppercase mb-1">ETA</p>
                <p className="text-[16px] font-bold text-emerald-400">{eta}</p>
              </div>
              <div className="w-px h-8 bg-[#2a2a2a]" />
              <div className="flex flex-col items-center">
                <p className="text-[#8b8b8b] text-[9px] font-bold tracking-widest uppercase mb-1">Dist</p>
                <p className="text-[16px] font-bold text-[#f5f5f5]">{distanceLeft}</p>
              </div>
              <div className="w-px h-8 bg-[#2a2a2a]" />
              <div className="flex flex-col items-center">
                <p className="text-[#8b8b8b] text-[9px] font-bold tracking-widest uppercase mb-1">Speed</p>
                <p className="text-[16px] font-bold text-[#f5f5f5]">{speed} kph</p>
              </div>
              {activeMissionId && (
                <>
                  <div className="w-px h-8 bg-[#2a2a2a]" />
                  <div className="flex flex-col items-center">
                    <p className="text-[#8b8b8b] text-[9px] font-bold tracking-widest uppercase mb-1">Mission</p>
                    <p className="text-[12px] font-bold text-emerald-400 font-mono">{activeMissionId}</p>
                  </div>
                </>
              )}
            </div>
        ...
        <div className="absolute bottom-6 right-4 z-30 flex flex-col items-center space-y-3">
          ...
          {/* BLOCK panic button */}
          <button
            className="w-14 h-14 bg-red-600 hover:bg-red-500 transition-colors rounded-full flex flex-col items-center justify-center shadow-[0_0_20px_rgba(220,38,38,0.4)] border-2 border-red-400"
            onClick={() => {
          ...
          >
            ...
            <span className="text-[8px] font-bold tracking-wider text-white">BLOCK</span>
          </button>
        </div>
